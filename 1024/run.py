...
# 默认使用shadowsocks的1080 socks v5端口
proxies = {
...
try:
  ...
  while flag <= DownloadPages:
    ...
    page_url = 'https://t66y.com/thread0806.php?fid=8&search=&page=' + \
        str(flag)
    # 获取第flag页的html
    get = requests.get(page_url, proxies=proxies)
    ...
    for url in article_url:
      threads = []
      title = ['default']
      getpage = requests.get(str(base_url) + str(url), proxies=proxies)
      ...
      if len(title) == 0:
        ...
        print('the content is ', str(getpage.content, 'gbk', errors='ignore'))
        continue
      file = './data/' + title[0]
